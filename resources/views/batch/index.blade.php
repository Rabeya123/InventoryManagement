                    ...
                    <div class="table-responsive">
                        <table class="table table-striped table-bordered table-hover" id="item-table" cellspacing="0" width="100%">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>{{ $ProductBatche->id }}</td>
                                    <td >{{ $ProductBatche->code  }}</td>
                                    <td>
                                        @if ($ProductBatche->sourceable_type == 'App\Models\PurchaseOrder')
                                            Purchase
                                        @elseif($ProductBatche->sourceable_type == 'App\Models\Inventory')
                                            Manual
                                        @endif
                                    </td>
                                    <td class="text-center">
                                        @if ($ProductBatche->is_active)
                                    ...
                                    <td class="text-center">
                                        N/A
                                        {{-- <a href="{{ route('products.edit', $ProductBatche->id)}}" class="btn btn-default btn-xs m-r-5" data-toggle="tooltip" data-original-title="Edit"><i class="fa fa-pencil font-14"></i></a>
                                        <form action="{{ route('products.destroy',$ProductBatche->id) }}" method="post" class="d-inline-block">
